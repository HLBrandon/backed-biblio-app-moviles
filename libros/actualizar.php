...
if (isset($_GET["id"]) and isset($_GET["isbn"]) and isset($_GET["titulo"]) and isset($_GET["autor"]) and isset($_GET["edit"]) and isset($_GET["anio"]) and isset($_GET["cat"]) and isset($_GET["stoke"])) {

    $id = trim($_GET["id"]); // Se necesita el ID del libro para actualizarlo
    $isbn = trim($_GET["isbn"]);
    ...
    // Verificar que se encontraron los IDs antes de actualizar
    if ($autor_id > 0 && $editorial_id > 0 && $categoria_id > 0) {
        // Actualizar el libro en la tabla libros
        $sql = "UPDATE libros 
                SET isbn={$isbn}, titulo={$titulo}, autor_id={$autor_id}, anio_publicacion={$anioPublicacion}, 
                    editorial_id={$editorial_id}, categoria_id={$categoria_id}, stoke={$stoke} 
                WHERE id={$id}";
